...
import { setLayoutNotify, setOrderUser, setOrderProduct } from 'src/store/actions'
...
import { STBillInfo, STContainer, STContent, STDescription, STHeader, STLeft, STPaymentInfo, STProduct, STPromotion, STRight } from './styled'
...
const Home:FC = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const isValid = await validate({ schema: homeSchema, data: { ...formData } })
      if (!isValid) return
  ...
  const PRODUCTS = useMemo(() => {
    return products?.map(item => <ProductItem key={item.name} data={item} handleCheckProduct={handleCheckProduct}/>)
  }, [products, handleCheckProduct])

  const product = useMemo(() => {
    return products?.find(({ checked }) => checked) || null
  }, [products])
    ...
    <STContainer>
      <Box maxWidth={1440} width="100%">
        ...
        <STContent>
          <STLeft>
            <STPromotion>
              <Typography variant="body2">
                Thời gian nhận ưu đãi cho những khách hàng đăng ký lần đầu chỉ còn:
              </Typography>
              <Typography variant="h4" component="span">{minutes.toString().padStart(2, '0')}:{seconds.toString().padStart(2, '0')}</Typography>
            </STPromotion>
            ...
            <STDescription>
              ...
              <Typography variant="body1">
                Đơn hàng của bạn dự kiến sẽ tới vào ngày <Typography variant="subtitle2" component="span" color="rgb(11, 154, 219)">6/12/2023</Typography>
              </Typography>
            </STDescription>
          ...
          <STRight>
            <STPaymentInfo>
              <Typography variant="h5" color={theme.colors['--color-white']}>THÔNG TIN THANH TOÁN</Typography>
              <Box
                width="100%"
                mt={3}
                p={3}
              ...
              >
                ...
                <Input
                  ...
                  error={errors.phone}
                />
                <Button fullWidth height={48} background={theme.colors['--color-positive-500']} colorText={theme.colors['--color-white']} onClick={handleSubmit}>
                  HOÀN TẤT ĐƠN HÀNG
                </Button>
              </Box>
            ...
            <STBillInfo>
              <Typography variant="h5" color={theme.colors['--color-black']}>THÔNG TIN HOÁ ĐƠN</Typography>
              <Box mt={1}>
                <Box display="flex" alignItems="center" justifyContent="space-between">
                  <Typography variant="body2">Tiền hàng:</Typography>
                  <Typography variant="subtitle2">{formatCurrency(product?.price || 0)}đ</Typography>
                </Box>
                <Box display="flex" alignItems="center" justifyContent="space-between">
                  <Typography variant="body2">Phí giao dịch/Phụ phí:</Typography>
                ...
                <Box display="flex" alignItems="center" justifyContent="space-between">
                  <Typography variant="body2" color={theme.colors['--color-black']}>Tổng đơn hàng:</Typography>
